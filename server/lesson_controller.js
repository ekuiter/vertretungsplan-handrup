...
LessonController._processLessonsHtml = function(lessonsType, lessonsHtml) {
  ...
  if (lastUpdated && lastUpdated.getTime() === updated.getTime()) {
    console.log("No refresh needed. Last updated: " + updated.getDate() + "." + (updated.getMonth() + 1) +
...
LessonController.stopScheduling = function() {
  if (LessonController._interval) {
    Meteor.clearInterval(LessonController._interval);
    LessonController._interval = null;
